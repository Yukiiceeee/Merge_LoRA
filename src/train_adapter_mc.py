...
class DataArguments:
    ...
    use_few_shot: bool = field(default=False, metadata={"help": "Whether to use few-shot examples in prompt"})
...
# 新增：为多选题创建优化的提示模板
def get_mc_prompt_template(use_few_shot=False):
...
class TaskDataset(Dataset):
    ...
    def __init__(self, 
        ...
        # 使用特定任务的提示模板
        if task == "mc":  # 多选题任务
            prompt_template = get_mc_prompt_template(use_few_shot)
            sources = [
...
def make_supervised_data_module(
    ...
    train_dataset = TaskDataset(
        ...
        task=data_args.task,
        use_few_shot=data_args.use_few_shot
    )
...
def train():
    ...
    # 保存任务类型信息，用于测试时的格式处理
    with open(os.path.join(training_args.output_dir, "task_info.json"), "w") as f:
        json.dump({
            "domain": data_args.domain,
            "task": data_args.task,
            "use_few_shot": data_args.use_few_shot
